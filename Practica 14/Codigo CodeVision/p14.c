...
unsigned char opc;
char anterior_a, cantidad_a, cantidad_b, anterior_b;
bit saltar = 0;
...
unsigned char calcular_intensidad(unsigned char valor, unsigned char cantidad, char modo) {
   ...
   for (i = 0; i < num_de_unos; i++) {
    if (modo == 1){//Hold
        if (cantidad == 0)//La primera vez se llena completo
            intensidad = (intensidad << 1) | 0x01;
        else
            if(i == 0)
               intensidad = (intensidad << 1) | 0x01;
            else if(i == 1)
               intensidad = (intensidad << cantidad + 1);
            else
               intensidad = (intensidad << 1) | 0x01;
    }
    else if (modo == 2) {//Punto
        if (i == 0)
            intensidad = (intensidad << 1) | 0x01;
        else
...
while (1)
      {
        ...
        if (saltar) {//espera para mostrar cambios saltando el proceso de intensidad
            saltar = ~saltar;
            delay_ms(50);
            continue;
        }  
        ...
        if (valor2 == anterior_b) {
            if (cantidad_b < 8) cantidad_b++;
        } else
          cantidad_b = 0;
            
        intensidad2 = calcular_intensidad(valor2, cantidad_b, modo);
        PORTD = intensidad2;
        anterior_b = valor2;
